...
"""
Here we compute the correlation function SzSz for h=0.
We also compute the dynamical structure factor.
"""
L = 10 if len(sys.argv)<2 else int(sys.argv[1]) #linear system size -> smaller now because it takes longer
Ns = L*L
UC = 1          #sites in UC
nkx = L//UC
nky = L
...
#Parameters
J_1 = 1 if len(sys.argv)<3 else int(sys.argv[2])        #J_1 = 1 -> AFM, J_1 = -1 -> FM
tit = "FM" if J_1 < 0 else "AFM"
delta = 0 if len(sys.argv)<4 else int(sys.argv[3])       #parameter for ZZ
sign = np.sign(J_1)
S = 0.5
dx = 5      #?
n_T = 31    #?
...
#
fn = "results/dssf_J"+"{:.1f}".format(J_1)+"_d"+"{:.1f}".format(delta)+"_t"+"{:.3f}".format(list_t[0])+"-"+"{:.3f}".format(list_t[-1])+"_"+str(len(list_t))+"_L"+str(L)+"_UC"+str(UC)+".npy"
save = False
plot_temp = True
if not Path(fn).is_file() or plot_temp:
    ...
    for ih in range(n_H):
        h = list_H[ih]
        ###
        th = 0 if delta==1 else fun_th(h,J_1,S)
        ...
        for iQ in range(lk):
            iQx,iQy = ind_path[iQ]
            ie = 0
            dssf[iQ,ie,0] = np.sin(th)**2*S/2*(uk[iQx,iQy]+vk[iQx,iQy])**2        #dssf zz
            dssf[iQ,ie,1] = w_h[iQx,iQy]        #energy
            dssfx[iQ,ie,0] = np.cos(th)**2*S/2*(uk[iQx,iQy]+vk[iQx,iQy])**2        #dssf xx
        ...
        ax = fig.add_subplot()
        sc = ax.imshow(im,cmap='inferno',aspect=0.7)
        #
