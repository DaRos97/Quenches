...
def system_function(t,y,*args):
    ...
    Hk = y[L**2+2:]
    #Parateres
    pxx1 = p_xx_1(th,ph,J1)
    pyy1 = p_yy_1(th,ph,J1)
    pzz1 = p_zz_1(th,ph,J1)
    pxy1 = p_xy_1(th,ph,J1)
    pxz1 = p_xz_1(th,ph,J1)
    pyz1 = p_yz_1(th,ph,J1)
    #Composite values
    eps = np.sum(Gk)/L**2
...
def p_xy_1(th,ph,J1):
    return -J1/2*np.sin(2*ph)*(1+np.cos(th)**2)
def p_xz_1(th,ph,J1):
...
def initial_condition(h_i,L):
    ...
    if 1:
        ...
        res[2:2+L**2] = np.random.rand(L**2)/10
    return res

def theta_canted_Neel(*args):
    J1,J2,D1,D2,h,S = args
    result = np.zeros(len(h))
    discriminant = 4*S*(J1*(1-D1)-J2*(1-D2))
    for i in range(len(h)):
        if abs(h[i])<abs(discriminant[i]):
            result[i] = np.arccos(h[i]/discriminant[i])
    return result
