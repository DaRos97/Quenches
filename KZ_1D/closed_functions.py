...
names = {   't-ev':'time_evolved_wf_',
            'fid':'fidelity_wf_',
            'pop':'populatins_wf_',
        }
def pars_name(tau,dt):
    return "{:.1f}".format(tau)+"_"+"{:.5f}".format(dt).replace('.',',')
...
def compute_fidelity(args):
    ...
    try:
        ...
        filename_psi = result_dir+names['t-ev']+name_pars+'.npy'
        try:
...
def compute_nex(ind_T,args):
    ...
    result_dir = home_dirname+dirname_closed
    #
    N = len(h_t)
    nex = []
    for n,Tau in enumerate(list_Tau):
        ...
        filename = result_dir+'nex_'+name_pars+'.npy'
        try:
            nex.append(np.load(filename))
        except:
            print("Computing nex of Tau = ",Tau," ...")
            #Find time evolved states
            filename_psi = result_dir+'time_evolved_wf_'+name_pars+'.npy'
            try:
                psi = np.load(filename_psi)
            except:
...
def compute_energy_state(args):
    h_t,J_t,times_dic,list_Tau,home_dirname,save_data = args
    result_dirme = home_dirname+dirname_closed
    #
    N = len(h_t)
    #
